...
---
# Erste Schritte mit Eventstream in Echtzeitanalyse (RTA)

Eventstream ist ein Feature in Microsoft Fabric, das Echtzeitereignisse ohne Code erfasst, transformiert und an verschiedene Ziele weiterleitet. Wenn Sie ein Eventstream-Element im Portal erstellen, handelt es sich um eine Instanz von Fabric-Eventstreams (auch als Eventstream bezeichnet). Sie können dem Eventstream Ereignisdatenquellen, Routingziele und den Ereignisprozessor hinzufügen, wenn die Transformation erforderlich ist. EventStore von Azure Service Fabric ist eine Überwachungsoption, die Ereignisse aus dem Cluster verwaltet und eine Möglichkeit bietet, den Status Ihres Clusters oder Ihrer Workloads zu einem bestimmten Zeitpunkt zu verstehen. Der EventStore-Dienst kann nach Ereignissen abgefragt werden, die in Ihrem Cluster für jede Entität und jeden Entitätstyp verfügbar sind. Dies bedeutet, dass Sie Ereignisse auf verschiedenen Ebenen abfragen können (z. B. Cluster, Knoten, Anwendungen, Dienste, Partitionen und Partitionsreplikate). Der EventStore-Dienst hat auch die Möglichkeit, Ereignisse in Ihrem Cluster zu korrelieren. Durch den Blick auf Ereignisse, die gleichzeitig von unterschiedlichen Entitäten geschrieben wurden und sich möglicherweise gegenseitig beeinträchtigt haben, kann der EventStore-Dienst diese Ereignisse verknüpfen und beim Identifizieren von Ursachen für Aktivitäten in Ihrem Cluster helfen. Eine weitere Option zur Überwachung und Diagnose von Azure Service Fabric-Clustern ist das Aggregieren und Sammeln von Ereignissen mithilfe von EventFlow.
...
Mit Microsoft Fabric-Eventstreams können Sie Ihre Ereignisdaten ganz einfach an einem Ort verwalten. Sie können Echtzeitereignisdaten in dem gewünschten Format sammeln, transformieren und an verschiedene Ziele senden. Sie können Ihre Eventstreams auch problemlos mit Azure Event Hubs, der KQL-Datenbank und Lakehouses verbinden.
...
Sie verwenden die Streaming- und Abfragefunktionen des Echtzeitanalysefeatures, um wichtige Fragen zur Aktienstatistik zu beantworten und die Ergebnisse zum Erstellen von Power BI-Berichten zu verwenden. In diesem Szenario nutzen Sie den Assistenten in vollem Umfang, anstatt einige Komponenten unabhängig voneinander zu erstellen (z. B. die KQL-Datenbank).
...
Als Nächstes aktivieren Sie die Verfügbarkeit in OneLake.

1. Wählen Sie im Bereich **Datenbankdetails** das Stiftsymbol aus.
...
3. Sie werden aufgefordert, den **Eventstream zu benennen**.

![Benennen des Eventstreams](./Images/name-eventstream.png)

4. Geben Sie dem Eventstream einen Namen, den Sie sich gut merken können (z. B. ***MyStockES**), und klicken Sie auf die Schaltfläche **Erstellen**.

## Eventstream-Quelldaten
...
## Ziel für Eventstream-Daten

1. Klicken Sie im Eventstream-Canvas auf **Neues Ziel**, und wählen Sie dann **KQL-Datenbank** aus.

![Eventstream-Ziel](./Images/new-kql-destination.png)

2. Verwenden Sie bei der Konfiguration der KQL-Datenbank die folgende Tabelle, um die Konfiguration abzuschließen.
...
> !Hinweis: Es kann erforderlich sein, die Seite zu aktualisieren, um Ihre Tabelle anzuzeigen, nachdem die Eventstream-Verbindung hergestellt wurde.
...
> ! Hinweis: Der KQL-Editor enthält sowohl Syntax als auch Inellisense-Hervorhebungen, mit denen Sie sich schnell Kenntnisse im Zusammenhang mit der Kusto-Abfragesprache (KQL) aneignen können.
...
> !Hinweis: Beachten Sie, dass die Volumes der Streamingdaten die Abfragegrenzwerte überschreiten. Dieses Verhalten kann abhängig von der Datenmenge variieren, die in Ihre Datenbank gestreamt wird.
...
1. In dieser Übung haben Sie eine KQL-Datenbank erstellt und kontinuierliches Streaming mit Eventstream eingerichtet. Danach haben Sie die Daten mit KQL und SQL abgefragt.
2. Wenn Sie Ihre KQL-Datenbank überprüft haben, können Sie den Arbeitsbereich löschen, den Sie für diese Übung erstellt haben. 
3. Wählen Sie auf der Leiste auf der linken Seite das Symbol für Ihren Arbeitsbereich aus.
4. Wählen Sie im Menü „...“ auf der Symbolleiste die Option „Arbeitsbereichseinstellungen“ aus.
5. Wählen Sie im Abschnitt „Andere“ die Option „Diesen Arbeitsbereich entfernen“ aus.